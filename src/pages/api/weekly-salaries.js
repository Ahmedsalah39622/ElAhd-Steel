...
async function handler(req, res) {
  ...
  try {
    ...
    // Calculate weekly salary for each worker
    const weeklySummary = workers.map(worker => {
      ...
      const weeklySalary = daysWorked * dailyRate
    ...
    // Calculate totals
    const totalDaysWorked = weeklySummary.reduce((sum, w) => sum + w.daysWorked, 0)
    const totalWeeklySalary = weeklySummary.reduce((sum, w) => sum + w.weeklySalary, 0)

    res.status(200).json({
      success: true,
      data: {
        workers: weeklySummary,
        summary: {
      ...
      message: 'Weekly salaries calculated successfully'
    })
  } catch (error) {
    console.error('Error calculating weekly salaries:', error)
    res.status(500).json({
      success: false,
      message: 'Failed to calculate weekly salaries',
      error: error.message
    })
  }
}
