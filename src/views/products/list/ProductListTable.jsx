...
const productStatusObj = {
  Scheduled: { title: 'مجدول', color: 'warning' },
  Published: { title: 'منشور', color: 'success' },
  Inactive: { title: 'غير نشط', color: 'error' }
}
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('category', {
        header: 'الفئة',
        cell: ({ row }) => {
          const categoryData = productCategoryObj[row.original.category] || productCategoryObj['Electronics']

          return (
            <div className='flex items-center gap-4'>
              <CustomAvatar skin='light' color={categoryData.color} size={30}>
                <i className={classnames(categoryData.icon, 'text-lg')} />
              </CustomAvatar>
              <Typography color='text.primary'>{row.original.category || 'N/A'}</Typography>
            </div>
          )
        }
      }),
      ...
      columnHelper.accessor('length', {
        header: 'الطول',
        cell: ({ row }) => <Typography>{row.original.length ?? '-'}</Typography>
      }),
      columnHelper.accessor('width', {
        header: 'القطر',
        cell: ({ row }) => <Typography>{row.original.width ?? '-'}</Typography>
      }),
      ...
      columnHelper.accessor('status', {
        header: 'الحالة',
        cell: ({ row }) => {
          const statusData = productStatusObj[row.original.status] || productStatusObj['Scheduled']

          return <Chip label={statusData.title} variant='tonal' color={statusData.color} size='small' />
        }
      }),
